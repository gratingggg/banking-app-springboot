...
public class CustomerService {
    ...
    private void setUniqueFields(Customer customer, CustomerRequestDTO customerRequestDTO){
        if(customerRequestDTO.getEmail() != null){
            if(customerRepository.findByEmail(customerRequestDTO.getEmail()).isPresent()){
                throw new IllegalArgumentException("The email " + customerRequestDTO.getEmail() + " is already registered. Please enter another email.");
            }
        ...
        if(customerRequestDTO.getPhoneNumber() != null){
            if(customerRepository.findByPhoneNumber(customerRequestDTO.getPhoneNumber()).isPresent()){
                throw new IllegalArgumentException("The phone number " + customerRequestDTO.getPhoneNumber() + " is already registered. Please enter another phone number.");
            }
        ...
        if(customerRequestDTO.getAadharNo() != null){
            if(customerRepository.findByAadharNo(customerRequestDTO.getAadharNo()).isPresent()){
                throw new IllegalArgumentException("The aadhar number " + customerRequestDTO.getAadharNo() + " is already registered. Please enter another aadhar number.");
            }
        ...
        if(customerRequestDTO.getUsername() != null){
            if (customerRepository.findByUsername(customerRequestDTO.getUsername()).isPresent()) {
                throw new IllegalArgumentException("The username " + customerRequestDTO.getUsername() + " is already taken. Please choose another username.");
            }
    ...
    public CustomerResponseDTO processCustomerLogin(CustomerLoginDTO customerLoginDTO){
        if(customerLoginDTO.getUsername() != null || customerLoginDTO.getUsername().isBlank()){
            Customer customer = customerRepository.findByUsername(customerLoginDTO.getUsername())
                    .orElseThrow(() -> new IllegalStateException("The employee with the username " + customerLoginDTO.getUsername() + " does not exist." +
